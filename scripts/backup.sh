...
if git init -q -b main "$WORK" \
   ...
   && git -C "$WORK" add -A \
   && git -C "$WORK" -c user.name=blog-backup -c user.email=blog-backup@localhost commit -qm "backup $TS" \
   && timeout 180 git -C "$WORK" push --force "$BACKUP_REPO_URL" main:main > /tmp/backup-push.log 2>&1; then
  ok '私有仓库快照' "push 成功（快照含 $(ls -1 "$WORK" | grep -vc '^\.git') 个文件）"
else
  fail '私有仓库快照' "$(tail -c 200 /tmp/backup-push.log | tr '\r' ' ' | tr '\n' ' ')（本地 14 天滚动备份不受影响）"
fi
rm -rf "$WORK"
finish ""
