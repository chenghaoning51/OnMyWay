#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""deploy-hook.py — 阶段 2.2 GitHub/Gitee Webhook 接收器（单文件，仅标准库）。
...
安全边界：
  ...
  * 验签二选一（定长比较）：GitHub X-Hub-Signature-256 = sha256=HMAC(secret, body)；
    Gitee X-Gitee-Token = 配置的 WebHook 密码明文（Gitee「密码」验证方式）
  * 只接受 X-GitHub-Event: push 或 X-Gitee-Event: Push Hook 且 ref == refs/heads/main；
    其余回 200 ignored（免得平台一直重试）
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if gh_sig:
            ok = hmac.compare_digest(gh_sig, signature(CFG["secret"], body))
            src = "github"
        elif gitee_token:
            ok = hmac.compare_digest(gitee_token.encode("utf-8"), CFG["secret"])
            src = "gitee"
        else:
            ok, src = False, "none"
        if not ok:
            STATE["rejected"] += 1
            log("REJECT 验签失败 src=%s 来自 %s" % (src, self.client_address[0]))
            return self._reply(403, "invalid signature")
        gh_event = self.headers.get("X-GitHub-Event", "")
        gt_event = self.headers.get("X-Gitee-Event", "")
        if gh_event != "push" and gt_event != "Push Hook":
            STATE["ignored"] += 1
            return self._reply(200, "ignored: event=%s/%s" % (gh_event or "-", gt_event or "-"))
        try:
        ...
        # GitHub 删分支带 deleted=true；Gitee 删分支 after=000…0。两者都不该触发发布
        if ref != CFG["ref"] or payload.get("deleted") is True or after == "0" * 40:
            STATE["ignored"] += 1
